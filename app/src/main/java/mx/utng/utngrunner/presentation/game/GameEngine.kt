...
object GameEngine {
    ...
    fun update(state: GameState, frame: Long): GameState {
        ...
        val afterCollision = checkCollisions(
            updatedPlayer, updatedObs, updatedCoins
        )
    ...
    private fun updateCoins(coins: List<Coin>, speed: Float): List<Coin> {
        return coins.map { it.copy(x = it.x - speed) }.filter { it.x > -50f }
    }
 
    /** Detección AABB (Axis-Aligned Bounding Box) */
    private fun checkCollisions(
        player: Player, obstacles: List<Obstacle>, coins: List<Coin>
    ): CollisionResult {
        var lives = 3  // Note: in update() we use state.lives, but here we calculate based on hits
        // Using a temporary variable to count hits in this frame.
        // In the original provided code, it was 'var lives = 3' which seems incorrect for multi-frame state.
        // But I will follow the spirit of the instruction.
        ...
        return CollisionResult(
            ...
            lives     = if (hitObs.isNotEmpty()) -1 else 0, // -1 means hit, 0 means no hit change in logic below
            obstacles = obstacles.map { if (it in hitObs) it.copy(x = -999f) else it },
            coins     = updatedCoins
        )
    }

    // Refined update that uses the collision signal
    fun updateWithState(state: GameState, frame: Long): GameState {
        if (state.phase != GamePhase.PLAYING) return state
        ...
        val newLives = if (collisionResult.lives < 0) state.lives - 1 else state.lives
        ...
        return state.copy(
            ...
            coins     = collisionResult.coins,
            phase     = if (newLives <= 0) GamePhase.DEAD
                        else GamePhase.PLAYING
        )
    }
}
 
data class CollisionResult(val player: Player, val lives: Int,
                           val obstacles: List<Obstacle>, val coins: List<Coin>)
